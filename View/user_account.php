    ...
    <div class="row" style="margin-left: 0.5%; margin-right: 0.5%;">
      ...
      <div class="col-lg-9 bg-light rounded">
            ...
            if ($user_acc_result->num_rows > 0) {
              while ($user_acc = $user_acc_result->fetch_assoc()) {
                ...
                <td>" . $user_acc["f_name"] . " " . $user_acc["m_name"] . " " . $user_acc["l_name"] . "</td> 
                    ...
                    <td>" . $row["status"] . "</td>                   
                    <td>                        
                        <a href='http://localhost/hrms/update/update_account.php?uname=$uname'>
                        <button class='btn-primary rounded'>Update</button>
                        </a>";
                if ($btn == "Active") {
                  echo "<a href='http://localhost/hrms/update/de_activate_account.php?uname=$uname&action=$btn'>
              <button class='btn-success rounded'>Deactivate</button></a></td>";
                } else {
                  echo "<a href='http://localhost/hrms/update/de_activate_account.php?uname=$stud_id&action=$btn'>
        ...
        <div class="text-center col=md 12">
          <?php
          if ($page > 1) {
            echo "<a href='http://localhost/hrms/View/user_account.php?page=" . ($page - 1) . "' class='btn btn-danger'>Previous </a>";
          }

          for ($i = 1; $i < $total_page; $i++) {
            echo "<a href='http://localhost/hrms/View/user_account.php?page=" . $i . "' class='btn btn-primary'>$i</a>";
          }

          if ($i > $page) {
            echo "<a href='http://localhost/hrms/View/user_account.php?page=" . ($page + 1) . "' class='btn btn-danger'> Next</a>";
          }
          ?>
          <hr>
        </div>
